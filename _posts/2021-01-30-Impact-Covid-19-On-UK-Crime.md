...
So, for example, possession of cannabis (weighted 3) is at the bottom end of the scale, along with e.g. dishonest use of electricity (weighted 7). Whilst homicide has the highest weight of 7,979. For each city and each year, we collected the crimes, multiplied each crime by the severity score for its category and them summed them, finally dividing by the population. 
